...
const vec3 light_pos = { 20.0f, 60.0f, 20.0f };
...
int main() {
   ...
   // puts("\033[91m");
    for (int y = 5; y < 50; y++) {
        for (int x = 0; x < 140; x++)
          {  //putchar("  .,-:;+=*pier#@"[(int)(f(make2((x / 160.0f - 0.5f) * 2.0f, (y / 80.0f - 0.5f) * -2.0f)) * 12.0f)]);
		putchar("  .,-:;+=*#@"[(int)(f(make2((x / 140.0f - 0.5f) * 2.0f, (y / 50.0f - 0.5f) * -2.0f)) * 12.0f)]);
